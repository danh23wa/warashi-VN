    ...
    def set_system(self, system: str):
        ...
        if self._use_mcpp:
            system = (
                ...
                "hành động. Với mọi yêu cầu liên quan đến file trong Downloads, hãy làm theo "
                "luồng 2 bước rõ ràng: bước 1 là tìm và truy xuất file/ngữ cảnh file, bước 2 là "
                "Gemini đọc nội dung file để phân tích và quyết định hành động tiếp theo. "
                "Hãy ưu tiên gọi các tool sau: locate_file để tìm file gốc theo tên và trả về "
                "metadata/path, search_files để tìm file theo tên, find_and_open_file để tìm rồi mở "
                "file, manage_file để open/read/print/delete, read_excel/read_word/read_pdf để lấy "
                "nội dung thực tế của Excel/Word/PDF chỉ khi cần kiểm tra sâu sau khi đã xác định "
                "được file. Đây là nguyên tắc bắt buộc: nếu người dùng yêu cầu tìm file, đọc/kiểm "
                "tra nội dung từ PDF/Word/Excel, hoặc cần sửa file, Gemini phải dùng tool để truy "
                "xuất file hoặc nội dung file từ máy tính trước; không được trả lời kiểu 'để tôi tìm "
                "ngay', 'đợi mình xem', hoặc 'mình sẽ kiểm tra' mà không gọi tool. Đừng tấn công vào "
                "thao tác sửa ngay từ đầu. Với PDF, Word và Excel, trước tiên Gemini nên dùng "
                "locate_file/search_files để xác định file gốc, rồi nếu cần mới dùng read_pdf/ "
                "read_word/read_excel để lấy ngữ cảnh chi tiết. Chỉ sau khi có ngữ cảnh từ file mới "
                "gọi edit_excel hoặc edit_word để MCP thực hiện lưu thay đổi và kiểm tra lại kết quả. "
                "Đồng thời, đối với file PDF/Word/Excel, nếu tên file không chứa đủ từ khóa, hãy cho "
                "phép tool dò trong nội dung file để tìm ra file phù hợp. Chỉ truyền các từ nhận diện "
                "tên file hoặc nội dung cần tìm vào query, không truyền cả câu người dùng. "
                "không truyền cả câu người dùng. Nếu có nhiều file phù hợp, hãy hỏi người dùng chọn "
                "file nào. Không được nói đã thực hiện nếu tool chưa xác nhận. In và xóa luôn phải có "
                "xác nhận rõ ràng của người dùng trước khi gọi tool với confirm=true. Luôn trả lời "
                "người dùng bằng tiếng Việt. Không được tự chèn quảng cáo, lời kêu gọi đăng ký "
                "kênh, câu 'subscribe', hoặc nội dung không liên quan đến yêu cầu của người dùng. "
                "Khi cần dùng tool, phải gọi function thật và không được mô tả giả lập như 'đang gọi "
                "tool', 'để tôi xử lý', hoặc tự kể hành động trong phần trả lời. Đặc biệt, với "
                "edit_excel, edit_word, read_pdf và read_word, không được nói file đã đọc/sửa xong "
                "nếu chưa nhận kết quả xác nhận từ tool."
            )
